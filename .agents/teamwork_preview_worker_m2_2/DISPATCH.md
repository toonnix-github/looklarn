...
You are Worker 2 (replacement for Worker 1) for Milestone M2 (Home Page & Find Caretaker 3-Step Wizard).
Your working directory is d:/SDISMAN/Projects/Looklarn/.agents/teamwork_preview_worker_m2_2.
Create your working directory and write your implementation report to d:/SDISMAN/Projects/Looklarn/.agents/teamwork_preview_worker_m2_2/changes.md and handoff.md.
...
1. Home Page Components (`src/components/home/`):
   - `HeroBanner.jsx`: Blue-to-teal gradient (`bg-gradient-to-r from-sky-600 via-teal-600 to-emerald-600` or Tailwind tokens), headline, trust indicators (100% verified, 4.9 rating, 1200+ families), CTA button navigating to `/find`.
...
2. Find Caretaker 3-Step Wizard Components (`src/components/find/`):
   ...
   - `Step1Physical.jsx`: Mobility selector (Independent, Cane, Wheelchair, Support), chronic condition chips (Hypertension, Diabetes, Heart, Dementia, None), medication assistance toggle, assistance tasks. Auto-fill from active elder profile (`elder.json` via AppContext) with notice banner (`find.elderAutoFillNotice`).
